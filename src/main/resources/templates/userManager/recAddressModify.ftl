		...
		<script type="text/javascript">
            $(function () {
				...
				$("#addRecAddress").click(function () {
					var recAddressAdd = $("input:text,input:hidden,select,textarea").serialize();
					...
					$.get("${request.contextPath}/userManager/doRecAddressModify?recAddressAdd="+recAddressAdd+"&areaId="+areaId,"isDefault="+isDefault,function (data) {
						if(data){
                            $("#message").hide();
							...
							<div class="user-profile-wrap">
                                ...
                                <div class="sui-form">
                                    <div class="address">
                                        ...
                                        <select id="provinceId">
											...
											<#list addressList as pro>
												<option value="${pro.provinceCode}" <#if pro.provinceId == recAddress.areas.cities.provinces.provinceId> selected = "selected"</#if>>
													${pro.province}
												</option>
											</#list>
                                        </select>
                                        <select id="cityId">
											...
											<#list addressList as pro>
												<#if pro.provinceId == recAddress.areas.cities.provinces.provinceId>
													<#list pro.citieses as cities>
														<option value="${cities.cityCode}" <#if cities.cityId == recAddress.areas.cities.cityId> selected = "selected"</#if>>
															${cities.city}
														</option>
													</#list>
												</#if>
                                        ...
                                        <select id="areaId" name="areaId">
											...
											<#list addressList as pro>
												<#if pro.provinceId == recAddress.areas.cities.provinces.provinceId>
													<#list pro.citieses as cities>
														<#if cities.cityId == recAddress.areas.cities.cityId>
															<#list cities.areases as area>
																<option value="${area.areaId}" <#if area.areaId == recAddress.areas.areaId> selected = "selected"</#if>>
																	${area.area}
																</option>
															</#list>
														</#if>
                                    ...
                                    <div style="margin-left: 150px;margin-top:-40px;">
                                        <textarea style="width:500px;height: 90px;padding: 5px;" id="recAddress" name="recAddress" placeholder="建议您如实填写详细收货地址，例如街道 名称，门牌号码，楼层和房间号等信息"><#if recAddress.recAddress??>${recAddress.recAddress}</#if></textarea>
                                    </div>

                                    <div style="margin-left: 80px;margin-top: 20px;">
                                        <span class="titles">邮政编号:</span>
                                        <input type="text" name="addressCode" value="<#if recAddress.addressCode??>${recAddress.addressCode}</#if>" placeholder="如您不清楚地区邮递号，请填写000000" style="padding: 5px;width: 300px;margin-left: 14px;" />
                                    </div>

                                    <div style="margin-left: 55px;margin-top: 30px;">
                                        <span style="color: #F2873B;">*</span>
                                        <span class="titles">收货人姓名:</span>
                                        <input type="text" name="recName" value="<#if recAddress.recName??>${recAddress.recName}</#if>" placeholder="长度不超过25个字符" style="padding: 5px;width: 300px;margin-left: 14px;" />
                                    </div>
                                    <div style="margin-left: 67px;margin-top: 30px;">
                                        ...
                                        <input type="text" name="phone" value="<#if recAddress.phone??>${recAddress.phone}</#if>" placeholder="电话号码必须为11位数字"  style="padding: 5px;width: 200px;"/>
                                    </div>

                                    <div style="margin-left: 150px;margin-top: 10px;">
                                        <div class="am-u-sm-7 am-u-sm-offset-3" style="padding-right: 0rem;">
                                            <label class="checkbox-pretty inline <#if recAddress.isDefault==1>checked</#if> ">
                                                <input style="color: #33302b;width:20px;heigth:20px" type="checkbox" <#if recAddress.isDefault==1>checked="checked"</#if> name="isDefault">
                                                <span style="font-size: 12px;color: #878787;">
                                                    <font style="font-size: 12px;color: #333333;">设为默认地址</font>
                                                </span> </label> </div>
                                    </div>
									<input type="hidden" value="${recAddress.recAddressId}" name="recAddressId"/>
							        <button id="addRecAddress" style="margin-left:150px;margin-top:10px;background-color:#F37B1D ;color: #fff;width: 100px;height: 30px;border: 0px;border-radius: 5px;">确定</button>
									<span id="message" style="color: red;font-size: 12px;line-height: 25px;display: none;">收货地址修改失败，请检查输入内容是否有误！</span>
                                </div>
							</div>
