    ...
    <script type="text/javascript">
        ...
        $(function () {
            ...
            $(".yz-input3").click(function () {
                userId = $(".yz-input1>input").val();
                userType = $(".yz-input2>select").val();
                pageNo = $("input:hidden[name=pageNo]").val();
                window.location.href=
                        "${request.contextPath}/superManager/superUserList?userId="+userId+"&userType="+userType;
            });
            //分页
            $("#first_page").click(function(){
                ...
                if(pageNo > 1){
                    ...
                    $("#current").val(pageNo);
                    window.location.href=
                            "${request.contextPath}/superManager/superUserList?pageNo="+pageNo+"&userId="+userId+"&userType="+userType;
                }
            })
        ...
        function userModify(index) {
            var userId = $(".list-superUser-body-td1:eq("+index+")").html();
            var userType = $(".list-superUser-body-td4:eq('"+index+"')>select").val();
            var integra = $(".list-superUser-body-td3:eq('"+index+"')>input").val();
            $.get("${request.contextPath}/superManager/modifyUser?userId="+userId+"&userType="+userType+"&integra="+integra,
                function (data) {
                    if (data){
                        window.location.href="${request.contextPath}/superManager/superUserList"
                    } else {
                        alert("修改失败,请检查输入信息！");
                    }
                });
    ...
    <div class="cont">
            ...
            <div class="ms-side">
                <article class="side-menu side-menu-off">
                    <dl class="side-menu-tree">
                        ...
                        <dd><a href="${request.contextPath}/superManager/superUserList" style="color:#f70"><span class="active"></span>用户管理</a></dd>
        ...
        <div class="yz-table2-div-box">
            ...
            <div  style="margin-top: 5px">
                <form>
                    <div >
                        <div class="yz-input1">用户ID：<input type="text" value="<#if userId?? && userId != 0>${userId}</#if>" name="userId"/></div>
                        <div class="yz-input2">用户类型：
                            <select>
                                <option value="0" <#if userType==0 >selected="selected"</#if> >未选择</option>
                                <option value="1" <#if userType==1>selected="selected"</#if> >买家用户</option>
                                <option value="2" <#if userType==2>selected="selected"</#if>>商家用户</option>
                                <option value="3" <#if userType==3>selected="selected"</#if>>管理员用户</option>
                            </select>
                        </div>
                    </div>
                    <div class="yz-bt"><input class="yz-input3" type="button" value="搜索用户" ></div>
                </form>
            </div>
            <table class="yz-table2-table">
                ...
                <table class="yz-table2-title">
                    ...
                    <tr class="list-table-tr1">
                        <td class="list-superUser-td1">用户ID</td>
                        <td class="list-superUser-td2">用户名</td>
                        <td class="list-superUser-td2">真实姓名</td>
                        <td class="list-superUser-td2">手机号码</td>
                        <td class="list-superUser-td4">用户类型</td>
                        <td class="list-superUser-td3">当前积分</td>
                        <td class="list-superUser-td1">操作</td>
                    </tr>
                    </tbody>
                </table>
                </thead>
                   ...
                   <#list pageInfo.list as user>
                       <div class="yz-super-table-div">
                           <table>
                               ...
                               <form>
                                   <td class="list-superUser-body-td1">${user.userId}</td>
                                   <td class="list-superUser-body-td2"><a href=""><#if user.userName??>${user.userName}</#if></a></td>
                                   <td class="list-superUser-body-td2"><#if user.realName??>${user.realName}</#if></td>
                                   <td class="list-superUser-body-td2"><#if user.phone??>${user.phone}</#if></td>
                                   <td class="list-superUser-body-td4">
                                       <select>
                                           <option value="1" <#if user.userType==1>selected="selected"</#if> >买家用户</option>
                                           <option value="2" <#if user.userType==2>selected="selected"</#if> >商家用户</option>
                                           <option value="3" <#if user.userType==3>selected="selected"</#if> >管理员用户</option>
                                       </select>
                                   </td>
                                   <td class="list-superUser-body-td3"><input name="" type="text" value="<#if user.integra??>${user.integra}"></#if></td>
                                   <td class="list-superUser-body-td11"><input onclick="userModify(${user_index})" type="button" value="修改"></td>
                               </form>
                               </tbody>
                           </table>
                       </div>
