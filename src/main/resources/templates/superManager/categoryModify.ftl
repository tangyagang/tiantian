    ...
    <script type="text/javascript">
        $(function () {
            $(".one_type").change(function () {
                ...
                $.get("${request.contextPath}/superManager/getProductCategory?parentId="+parentId,function (data) {
                    $(".two_type>option").remove();
                    for (var i=0;i<data.length;i++){
                        var productCategory = data[i];
                        $(".two_type").append(
                            "<option value="+productCategory.pcId+">"+productCategory.pcName+"</option>"
                        );
                    }
                });
            })
...
<div id="yz-center" class="personal-member">
    <div class="cont">
        ...
        <div class="yz-div-box">
            <dl>添加分类</dl>
            <div style="margin-top:4px;width: 900px;height: 1px;background-color: #f88600"></div>
            <div style="position:absolute;height: 150px;width: 1068px;top:50%;left:35%">
                <div>
                    <form action="${request.contextPath}/superManager/doCategoryModify" method="post" class="yz-category-from">
                        <table class="">
                            <tr>
                                <td class="">一级分类：</td>
                                <td>
                                    <select class="one_type" name="productCategory.parentId">
                                        <#list list as productCategory>
                                            <option value="${productCategory.pcId}"
                                                <#if pc.productCategory.productCategory.pcId == productCategory.pcId>selected="selected"</#if>
                                            >${productCategory.pcName}</option>
                                        </#list>
                                    </select>
                                </td>
                            </tr>
                            <tr >
                                <td class="">二级分类：</td>
                                <td>
                                    <select class="two_type" name="parentId">
                                        <#list list as productCategory1>
                                            <#if pc.productCategory.productCategory.pcId == productCategory1.pcId>
                                                <#list productCategory1.productCategorys as productCategory2>
                                                    <option value="${productCategory2.pcId}"
                                                        <#if pc.productCategory.pcId == productCategory2.pcId>selected="selected"</#if>
                                                    >${productCategory2.pcName}</option>
                                                </#list>
                                            </#if>
                                        </#list>
                                    </select>
                            ...
                            <tr>
                                <td class="">三级分类：</td>
                                <td>
                                    <input type="hidden" value="${pc.pcId}" name="pcId"/>
                                    <input type="text" value="${pc.pcName}" class="text" name="pcName"/>
                                    <#if message??><span style="color: red;font-size: 12px">${message}</span></#if>
                                </td>
                            </tr>
                            <tr>
                                <td></td>
